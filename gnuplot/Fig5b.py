...
for dname in dlist:
    ...
    if ('fb' in dname):
        lstl = '-'
        lbl = tlist[idl] + r" m, " + lonlist[idl]
    scolor = clist[idl] # next(ax._get_lines.color_cycle) #['color']
    im2 = ax2.plot(abs(lonL)*np.pi/180., colL, linestyle=lstl, clip_on=False, c=scolor, label=lbl)
    print ("Eq. pos: %.3f %.3f" % (lonLf[2], colLf[2]))
    if(lstl=='-'): im2 = ax2.plot(abs(lonLf[2])*np.pi/180., colLf[2], 'o', clip_on=False, c=scolor, markersize=10)
        
ax2.yaxis.set_major_formatter(ticker.FuncFormatter(degsymb))
ax2.set_title('b) Negative load, formed near SP')
ax2.set_thetamin(0)
ax2.set_thetamax(90)
ax2.legend(fontsize = 15, loc='lower right', bbox_to_anchor=(1.02, 0.76), framealpha=1.0)
ax2.grid(True)
...
ax2.scatter(0, 90,marker='+', s=100, clip_on=True, c='black', label='tidal axis')
ax2.scatter(4*np.pi/180, 66., marker='p', s=200, clip_on=False, edgecolors='black', facecolors='none', label='SP')
#plt.colorbar(im2,ax=ax2,format="%.0f", orientation="horizontal", label='time [My]',pad=0.5)
ax2.set_rmin(0.0)
ax2.set_rmax(70.) #(1.1*max(colL))
...
plt.savefig("../figs/Fig5b.png", bbox_inches='tight')
plt.clf()
